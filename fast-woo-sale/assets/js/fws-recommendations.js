...
(function($) {
  ...
  function fwsFormatPrice(amount) {
    // نسخهٔ ۲.۱۲.۷ (H-39): مقدار نامعتبر (NaN/Infinity) هیچ متنی تولید نمی‌کند —
    // قبلاً Number(amount) || 0 مقدار نامعتبر را به ۰ تبدیل می‌کرد و Infinity به
    // toFixed/toLocaleString می‌رسید و رشتهٔ بی‌معنی نمایش داده می‌شد.
    var __fws_num = Number(amount);
    if (!Number.isFinite(__fws_num)) return '';
    var f = (typeof fws_params.price_format === 'object' && fws_params.price_format) || null;
    if (!f) {
      var sym0 = fws_params.currency_symbol || 'تومان';
      return Math.round(__fws_num).toLocaleString('fa-IR') + ' ' + sym0;
    }
  ...
  /**
   * نسخهٔ ۲.۱۳ (I-46): اطلاع‌رسانی به Mini-Cart بلاکی. ووکامرس بلاکس سبدِ کوچکِ خود را
   * از Store API می‌خواند و به فرگمنت‌های کلاسیک بی‌اعتناست؛ رویدادِ پلِ رسمیِ آن
   * (wc-blocks_added_to_cart روی document.body) قبلاً صادر نمی‌شد و کارتِ بلاکی تا
   * رفرش صفحه محتوای قبلی را نشان می‌داد. jQuery added_to_cart هم برای سایر مصرف‌کنندگان
   * سر جایش است — این رویدادِ بومی «افزودنیِ» آن است، نه جایگزین.
   */
  function fwsNotifyBlockMiniCart() {
    if (document.body && typeof document.body.dispatchEvent === 'function') {
      try { document.body.dispatchEvent(new Event('wc-blocks_added_to_cart')); } catch (err) {}
    }
  }
  ...
  function fwsRecalcBundle($wrap) {
    ...
    var total = 0;
    // نسخهٔ ۲.۱۲.۷ (H-34): اگر PHP مبلغ خام را در data-price-raw (wc_format_decimal —
    // همیشه ممیزِ انگلیسی، بدون جداکنندهٔ هزارگان) گذاشته باشد، فقط همان خوانده می‌شود؛
    // data-price (قیمت نمایشی) فقط fallback است — جمعِ پیش‌نمایش بسته دیگر با هر
    // مقدار محلی‌شده (مثل 12,50 یا 1,200,000) گمراه نمی‌شود.
    $checked.each(function() {
      var __fws_raw = $(this).data('price-raw');
      if (typeof __fws_raw === 'undefined' || __fws_raw === '' || __fws_raw === null) {
        __fws_raw = $(this).data('price');
      }
      total += parseFloat(__fws_raw) || 0;
    });
  ...
  $(document).ready(function() {
    ...
    $(document).on('click', '.fws-thankyou-claim-btn', function(e) {
      ...
      // نسخهٔ ۲.۱۲.۶ (G-12): انقضای امضای رندر همراه درخواست ارسال می‌شود
      var upsellExp = $btn.data('upsell-exp') || 0;
    ...
    // نسخهٔ ۲.۱۲.۷ (H-40): دسترس‌پذیری مودال خروج — aria-modal بدون مدیریتِ فوکوس
    // کامل نیست: (۱) هنگام باز شدن، فوکوس به دکمهٔ بستن می‌رود؛ (۲) Tab داخل پنجره
    // قفل می‌شود (فوق‌بُعد آخرین عنصر به اولین، شیفتبُعد برعکس)؛ (۳) بعد از بستن،
    // فوکوس به عنصری که موقع باز شدن روی آن بود برمی‌گردد. ESC از قبل بود و در
    // همان هندلر یکپارچه شد.
    var fwsExitLastFocus = null;

    function fwsCloseExitModal() {
      var $m = $('#fws-exit-intent-modal');
      if (!$m.is(':visible')) return;
      $m.fadeOut(200);
      if (fwsExitLastFocus && fwsExitLastFocus.focus) {
        setTimeout(function() {
          try { fwsExitLastFocus.focus(); } catch(err) {}
        }, 210);
      }
      fwsExitLastFocus = null;
    }
    ...
    function fwsShowExitModal() {
      ...
      fwsExitLastFocus = document.activeElement;
      $exitModal.fadeIn(200);
      // فوکوس به داخل مودال — دکمهٔ بستن؛ اگر نبود خودِ کانتینر
      setTimeout(function() {
        var $target = $exitModal.find('.fws-modal-close').first();
        if (!$target.length) { $target = $exitModal; }
        $target.attr('tabindex', '-1').trigger('focus');
      }, 210);
      // نسخه ۲.۱۰: ثبت نمایش واقعی مودال خروج در گزارش درآمد (بیکن سبک، بی‌صدا)
      // نسخهٔ ۲.۱۳ (I-47): توکن یک‌بارمصرف اثباتِ نمایش — هنگام رندر مودال سرور آن را در
      // سشن ذخیره کرده و در data-exit-token منتشر کرده است؛ endpoint فقط با همان توکن ثبت
      // می‌کند و بعد از مصرف، توکن باطل است.
      if (fws_params.tracking_enable === 'yes') {
        fwsPost({ action: 'fws_track_event', track_type: 'exit_shown', exit_token: String($exitModal.data('exit-token') || '') });
      }
    }
    ...
    // نسخهٔ ۲.۱۲.۷ (H-40): ESC برای بستن + تلهٔ Tab داخل مودال
    $(document).on('keydown', function(e) {
      ...
      if (e.key === 'Tab' || e.keyCode === 9) {
        var $focusable = $m.find('a[href], button:not([disabled]), input:not([disabled]), select, textarea, [tabindex]:not([tabindex="-1"])').filter(':visible');
        ...
        var inside = active && $m.find(active).length > 0;
        if (e.shiftKey) {
