...
class FWS_Tracker {
    ...
    /**
     * نسخهٔ ۲.۱۲.۶ (G-14): کوکی هویت سشن — جانشین هش IP/UA.
     * توکن تصادفی ۳۲-کاراکتری (هگز) که فقط هویت تحلیلی ناشناس است و هیچ دادهٔ شخصی
     * ندارد؛ کاربران پشت یک NAT/شرکت دیگر در هم فشرده نمی‌شوند و شمارش سشن متمایز
     * و آمار A/B درست می‌ماند. با فیلتر fws_visitor_cookie_enabled قابل خاموش‌کردن
     * است (هویت به روش قبلی هش IP/UA سقوط می‌کند).
     */
    const VISITOR_COOKIE  = 'fws_visitor_id';
    // نسخهٔ ۲.۱۰.۴ — ارتقای نسخهٔ شِما برای افزودن ایندکس widget_time (widget, event_time)
    // نسخهٔ ۲.۱۲.۵ (F-40) — افزودن ایندکس user_id: خروجی/حذف GDPR صفحه‌صفحه با WHERE user_id
    // نسخهٔ ۲.۱۲.۶ (G-07) — افزودن ستون test_id + ایندکس: ردیف‌های رخدادِ A/B هویت تستِ خود
    // را حمل می‌کنند تا delete_test دیگر با بازهٔ زمانیِ مشترک، ردیف‌های تستِ هم‌پوشانِ
    // دیگر روی همان ویجت را قربانی نکند.
    // نسخهٔ ۲.۱۳ (I-59) — افزودن ستون currency: درآمد هر ردیف با ارزِ لحظهٔ ثبتش ذخیره
    // می‌شود تا تغییر ارز فروشگاه (مثلاً ریال ← تومان) در گزارش، مبلغ‌های قدیم و جدید
    // را بی‌صدا با هم جمع نکند.
    const DB_VERSION      = '2.13.0';
    ...
    /**
     * نسخهٔ ۲.۱۲.۶ (G-14): توکن کوکی سشن بازدیدکننده (اگر موجود/قابل‌صدور باشد).
     * کاربران پشت یک NAT (IP مشترک /24 یا حتی IP کامل) با هش IP|UA یکی شمرده
     * می‌شدند و شمارش سشن متمایز و آمار A/B خراب بود؛ توکن تصادفی مرورگری، هر
     * مرورگر واقعی را مستقل می‌کند. صدور فقط برای بازدیدکنندهٔ واقعیِ با ردیابی روشن
     * (بوت‌ها/ادمین‌ها کوکی نمی‌گیرند) و مقدار تصادفی است — هیچ دادهٔ شخصی ندارد.
     * @return string توکن ۳۲-کاراکتری یا رشتهٔ خالی (یعنی سقوط به روش IP/UA)
     */
    private static function visitor_cookie_identity() {
        if (!apply_filters('fws_visitor_cookie_enabled', self::tracking_enabled() && !self::should_exclude_visitor())) {
            return '';
        }
        $cookie = isset($_COOKIE[self::VISITOR_COOKIE]) ? (string) $_COOKIE[self::VISITOR_COOKIE] : '';
        if (1 === preg_match('/^[a-f0-9]{32}$/', $cookie)) {
            return $cookie; // کوکی معتبر موجود
        }
        if (headers_sent() || wp_doing_cron()) {
            return '';
        }
        $token = md5(wp_generate_password(32, true, true) . microtime(true) . wp_rand());
        if (!setcookie(self::VISITOR_COOKIE, $token, array(
        ...
        $_COOKIE[self::VISITOR_COOKIE] = $token; // همین درخواست هم همان هویت را ببیند
        return $token;
    }

    /**
     * هش یک‌طرفه سشن بازدیدکننده (نسخهٔ ۲.۱۲.۶: اول کوکی تصادفی، سپس ناشناس‌سازی IP)
     * @return string
     */
    public static function session_hash() {
        ...
        // نسخهٔ ۲.۱۲.۶ (G-14): هویت سشن اول از کوکی تصادفی — NAT دیگر بازدیدکننده‌های
        // متفاوت را یکی نمی‌کند؛ ردیابی تبدیل سشن به سشن از رندر تا پرداخت هم دقیق‌تر
        // می‌شود. سقوط امن به روش قبلی: بدون کوکی (فیلتر/هدرسنت/بوت/ردیابی خاموش).
        $cookie_identity = self::visitor_cookie_identity();
        if ('' !== $cookie_identity) {
            self::$session_hash = $cookie_identity; // ۳۲ هگز — هم‌نوع ستون CHAR(32)
            return self::$session_hash;
        }
    ...
    public static function create_tables() {
        ...
        $sql = "CREATE TABLE {$table} (
            ...
            test_id VARCHAR(32) NOT NULL DEFAULT '',
            PRIMARY KEY  (id),
    ...
    public static function init() {
        ...
        // نسخهٔ ۲.۱۲.۶ (G-05): بازپرداخت درآمد منتسب را کم می‌کند — پیش‌تر هیچ هوکی برای
        // refund وجود نداشت و پس از بازپرداخت، گزارش درآمد بیش از واقعیت می‌ماند.
        add_action('woocommerce_order_refunded', array(__CLASS__, 'attribute_refund'), 10, 2);
    ...
    private static function insert_event($type, $widget, $product_id = 0, $variant = '', $order_id = 0, $revenue = 0.0, $identity = null) {
        ...
        // نسخهٔ ۲.۱۲.۶ (G-07): مهر شناسهٔ تست A/B روی رخدادهای مرور (نمایش/افزودن/کوپن).
        // ردیف‌های خرید همیشه بدون test_id می‌مانند (variant آن‌ها از متای آیتم سفارش می‌آید
        // و به تستِ لحظهٔ کلیک تعلق دارد نه تستِ لحظهٔ انتساب) — پاکسازی تست، درآمد واقعی
        // سفارش‌ها را دست نمی‌زند.
        $test_id = '';
        if ('purchase' !== $type && in_array($variant, array('A', 'B'), true) && class_exists('FWS_AB_Testing')) {
            $running = FWS_AB_Testing::running_test_for($widget);
            if ($running && (string) $running['variant'] === (string) $variant) {
                $test_id = (string) $running['id'];
            }
        }
        ...
        $inserted = $wpdb->insert(
            ...
            array(
                ...
                // نسخهٔ ۲.۱۳ (I-59): ارزِ لحظهٔ ثبتِ ردیف — مبنای تفکیک درآمد در گزارش
                'currency'     => (function_exists('get_woocommerce_currency') ? (string) get_woocommerce_currency() : ''),
                'test_id'      => sanitize_text_field($test_id),
            ),
            array('%s', '%s', '%s', '%d', '%s', '%s', '%d', '%d', '%f', '%s', '%s')
        );
    ...
    public static function attribute_order($order_id) {
        ...
        // نسخهٔ ۲.۱۳ (I-56): تنظیم «مدیران را ردیابی نکن» حالا روی خریدها هم اعمال می‌شود.
        // قبلاً فقط بازدید/نمایشِ مدیرِ واردشده فیلتر می‌شد؛ سفارش آزمایشیِ او (مخصوصاً با
        // درگاه آفلاین که گذار وضعیتش دستی در پنل انجام می‌شود) به‌عنوان درآمد واقعی ثبت
        // و نرخ تبدیل گزارش را خراب می‌کرد. ملاک، «مشتریِ صاحب سفارش» است نه کاربرِ فعلی
        // درخواست — چون گذار وضعیت ممکن است توسط هر ادمین دیگری در پنل انجام شود.
        $fws_order_customer = (int) $order->get_customer_id();
        if ($fws_order_customer > 0
            && 'yes' === FWS_Settings::get('tracking_exclude_admins', 'yes')
            && user_can($fws_order_customer, 'manage_woocommerce')) {
            return; // خرید آزمایشی مدیرِ فروشگاه — درآمد محسوب نمی‌شود
        }
        ...
        // نسخهٔ ۲.۱۲.۵ (F-27): این گارد عملاً مرده بود — در سفارشِ قدیمی «همهٔ» آیتم‌ها
        // بی‌فلگ‌اند پس $pending_items هیچ‌وقت خالی نیست و شرط هرگز true نمی‌شد؛ نتیجه:
        // هر فراخوانی مجدد attribute_order (گذار cancelled→processing، آپسل legacy_append)
        // «کل» سفارش را برای بار دوم منتسب و درآمد گزارش را دوبرابر می‌کرد. اکنون در
        // اولین مواجهه، آیتم‌های قدیمی (متای منبع با املای پیش از ۲.۱۰.۲ — بدون زیرخط)
        // مهر «قبلاً منتسب» می‌خورند و فقط آیتم‌های با املای جدید (مثل آپسلی که همین
        // حالا اضافه شده) به حلقهٔ دلتایی می‌روند. مهرِ _fws_legacy_backfilled این
        // مهاجرت یک‌باره را قفل می‌کند.
        if ($legacy_done && !$has_item_flag && !$order->get_meta('_fws_legacy_backfilled', true)) {
            $still_pending = array();
            foreach ($pending_items as $item) {
                if ('' === (string) $item->get_meta('_fws_source', true)) {
                    $item->add_meta_data('_fws_attributed_item', current_time('mysql', true), true);
                } else {
                    $still_pending[] = $item;
                }
            }
            $order->update_meta_data('_fws_legacy_backfilled', current_time('mysql', true));
            $order->save();
    ...
    /**
     * نسخهٔ ۲.۱۲.۶ (G-05): کسر درآمدِ بازپرداخت‌شده از گزارش ویجت مبدأ.
     * هوک woocommerce_order_refunded برای بازپرداخت کامل و جزئی هر دو آتش می‌شود؛
     * برای هر خطِ بازپرداخت‌شده که آیتم اصلیِ آن «منتسب» بوده، یک ردیف خرید با مبلغ
     * «منفی» ثبت می‌شود (مبلغ خط در ردیف‌های بازپرداخت ووکامرس ذاتاً منفی است).
     * شمارش خریدها (COUNT(DISTINCT order_id)) تغییر نمی‌کند — خرید واقعاً رخ داده —
     * فقط درآمد گزارش به اندازهٔ بازپرداخت کم می‌شود و گزارش دیگر بیش از واقعیت نیست.
     * ردیف منفی هم test_id ندارد؛ حذف تست هرگز درآمد سفارش‌ها را پاک نمی‌کند.
     *
     * @param int $order_id  شناسه سفارش اصلی
     * @param int $refund_id شناسه ردیف بازپرداخت
     */
    public static function attribute_refund( $order_id, $refund_id = 0 ) {
        if ( ! self::tracking_enabled() ) {
            return;
        }
        $order  = wc_get_order( $order_id );
        $refund = ( $refund_id > 0 ) ? wc_get_order( $refund_id ) : null;
        if ( ! $order || ! method_exists( $order, 'get_items' ) || ! $refund ) {
            return;
        }

        $adjusted = false;
        foreach ( $refund->get_items() as $refund_item ) {
            // WC خط بازپرداخت را با متای _refunded_item_id به آیتم اصلی وصل می‌کند
            $refunded_item_id = (int) $refund_item->get_meta( '_refunded_item_id', true );
            if ( $refunded_item_id <= 0 ) {
                continue;
            }
            $item = $order->get_item( $refunded_item_id );
            if ( ! $item || ! is_a( $item, 'WC_Order_Item_Product' ) ) {
                continue;
            }
            if ( ! $item->get_meta( '_fws_attributed_item', true ) ) {
                continue; // آیتم منتسبِ افزونه نبود — کسری هم به گزارش ویجت مربوط نیست
            }
            $widget = (string) $item->get_meta( '_fws_source', true );
            if ( '' === $widget ) {
                $widget = (string) $item->get_meta( 'fws_source', true );
            ...
            $amount = (float) $refund_item->get_total();
            if ( $amount >= 0 ) {
                continue; // دادهٔ بازپرداخت غیرمنفی — چیزی برای کسر نیست
            }
            $variant = (string) $item->get_meta( '_fws_variant', true );
            if ( '' === $variant ) {
                $variant = (string) $item->get_meta( 'fws_variant', true );
    ...
    /**
     * نسخهٔ ۲.۱۲.۶ (G-06): حذف ردیف‌های درآمدِ یک سفارش — فقط برای مسیر شکستِ
     * قطعی‌سازیِ سفارش وابستهٔ آپسل (F-44): گذار وضعیت ممکن است قبل از شکست، ردیف‌های
     * خرید را ثبت کرده باشد؛ سفارشی که لغو می‌شود نباید درآمدِ ارواد در گزارش بگذارد.
     *
     * @param int $order_id
     * @return int تعداد ردیف‌های حذف‌شده
     */
    public static function delete_order_events( $order_id ) {
        global $wpdb;
        $order_id = absint( $order_id );
        if ( $order_id <= 0 || ! self::ensure_table() ) {
            return 0;
        }
        $table = $wpdb->prefix . self::TABLE_EVENT;
        $deleted = (int) $wpdb->query( $wpdb->prepare(
    ...
    public static function get_report($days = 30) {
        ...
        // نسخهٔ ۲.۱۳ (I-59): تفکیک درآمد بر حسب ارزِ لحظهٔ ثبت. جمعِ اصلی (totals.revenue)
        // فقط ردیف‌های هم‌ارزِ «ارز فعلی فروشگاه» را می‌شمارد؛ اگر در همین بازه ردیفی با
        // ارزِ دیگر (تغییر ارز فروشگاه، مثلاً ریال ← تومان) وجود داشته باشد، جمعِ خامِ آن
        // در revenue_by_currency می‌ماند و پرچم mixed_currency صادر می‌شود تا هیچ‌گاه
        // مبلغ‌های ناهم‌ارز بی‌صدا با هم جمع نشوند. فروشگاه تک‌ارزی: خروجی مثل قبل + آرایهٔ خالی.
        $current_currency = function_exists('get_woocommerce_currency') ? (string) get_woocommerce_currency() : '';
        $rev_by_currency  = array();
        $rev_rows         = $wpdb->get_results($wpdb->prepare(
            "SELECT currency, SUM(revenue) AS revenue
             FROM {$table}
             WHERE event_time >= %s AND event_type IN ('purchase','coupon')
             GROUP BY currency",
            $since
        ), ARRAY_A);
        if ('' !== $wpdb->last_error) {
        ...
        $mixed = false;
        foreach ((array) $rev_rows as $rev_row) {
            // نسخهٔ ۲.۱۳: ردیف‌های پیش از ارتقای شِما (currency='') عمداً با ارزِ فعلی «یکی»
            // فرض نمی‌شوند — فروشگاهی که ارز عوض کرده، آن ردیف‌ها ارزِ دیگری داشته‌اند؛
            // گروه جدا می‌مانند تا هرگز بی‌صدا با مبلغِ ناهم‌ارز جمع نشوند.
            $cur                   = (string) $rev_row['currency'];
            $rev_by_currency[$cur] = (float) $rev_by_currency[$cur] + (float) $rev_row['revenue'];
        }
        if (count($rev_by_currency) > 1 && '' !== $current_currency && isset($rev_by_currency[$current_currency])) {
            $mixed = true;
            $report['totals']['revenue'] = $rev_by_currency[$current_currency];
            // تفکیک ریز ویجت‌ها فقط از درآمد هم‌ارزِ فعلی؛ سهم سایر ارزها در revenue_by_currency
            $wcur_rows = $wpdb->get_results($wpdb->prepare(
                "SELECT widget, SUM(revenue) AS revenue
                 FROM {$table}
                 WHERE event_time >= %s AND event_type IN ('purchase','coupon') AND currency = %s
                 GROUP BY widget",
                $since,
                $current_currency
            ), ARRAY_A);
            $wmap = array();
            foreach ((array) $wcur_rows as $wr) {
                $wmap[(string) $wr['widget']] = (float) $wr['revenue'];
            }
            foreach (array_keys($report['widgets']) as $wkey) {
                $report['widgets'][$wkey]['revenue'] = isset($wmap[$wkey]) ? $wmap[$wkey] : 0.0;
            }
        } elseif (count($rev_by_currency) > 1) {
            $mixed = true;
        }
        $report['revenue_by_currency'] = $rev_by_currency;
        $report['mixed_currency']      = $mixed;
