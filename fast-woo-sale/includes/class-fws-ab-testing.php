...
class FWS_AB_Testing {
    ...
    public static function test_stats($test, $until_ts = 0) {
        ...
        // نسخهٔ ۲.۱۳ (I-45): وجود جدول حداکثر یک بار در هر درخواست بررسی می‌شود + مانند
        // ensure_table ردیاب (B-15) در آبجکت‌کش با TTL پنج‌دقیقه‌ای — قبلاً هر فراخوانی
        // (حتی از کرون روزانه برای هر تست فعال) یک SHOW TABLES تازه می‌زد.
        static $table_ok = null;
        if (null === $table_ok) {
            $table_ok = wp_cache_get('fws_ab_stats_table_ok', 'fws_db_checks');
            if (false === $table_ok) {
                $table_ok = ($wpdb->get_var($wpdb->prepare('SHOW TABLES LIKE %s', $table)) === $table) ? 1 : 0;
                wp_cache_set('fws_ab_stats_table_ok', $table_ok, 'fws_db_checks', 5 * MINUTE_IN_SECONDS);
            }
        }
        if (!$table_ok) {
            return $out;
        ...
        // نسخهٔ ۲.۱۳ (I-45): تشخیصِ «سقف صریح» پیش از بازنویسی — پنجرهٔ تستِ running تا
        // «اکنون» است و کلید کشش باید بِکتِ ۲ دقیقه‌ای بخورد؛ سقفِ صریح (کارت برندهٔ تازه)
        // دقیق کلید می‌خورد چون مقدارش ثابت است.
        $fws_explicit_until = ( $until_ts > 0 );
        $until_ts = $fws_explicit_until ? min((int) $until_ts, time()) : time();
        if ($until_ts < $since_ts) {
            $until_ts = $since_ts;
        }

        // نسخهٔ ۲.۱۳ (I-45): نتیجهٔ آمار هر تست در transient با بِکتِ ۲ دقیقه‌ای کش می‌شود.
        // پنجرهٔ تست‌های running تا «اکنون» است؛ کلید کش از کلیدِ زمانیِ ثابت‌شده (floor
        // به ۱۲۰ ثانیه) استفاده می‌کند تا کوئریِ سنگین COUNT(DISTINCT) در موجِ بازدیدهای
        // پنل/کرون فقط دوبار در دقیقه اجرا شود. سقف پنجرهٔ صریح (کارت برندهٔ تازه) دقیق کلید می‌خورد.
        $bucket     = $fws_explicit_until ? $until_ts : (time() - (time() % 120));
        $cache_key  = 'fws_ab_stats_' . md5((string) $test['widget'] . '|' . $since_ts . '|' . $bucket);
        $cached     = get_transient($cache_key);
        if (is_array($cached) && isset($cached['A']['impression'])) {
            return $cached;
        }
        ...
        // نتیجه (حتی خالی) تا دو دقیقه معتبر است — نرخ تبدیل آمارِ واقعی را خراب نمی‌کند؛
        // کرونِ قفل برنده روزانه است و گزارش پنل خودش کش ۵ دقیقه‌ای دارد.
        set_transient($cache_key, $out, 120);
        return $out;
    }
    ...
    public static function create_test($widget_slug, $title_b, $title_a, $color_b, $split) {
        ...
        // نسخهٔ ۲.۱۳ (I-48): تست بدون ردیابی = هیچ رخدادی ثبت نمی‌شود؛ تست هرگز به نتیجه
        // نمی‌رسد ولی یکی از ظرفیت ۱۲تایی تست را برای همیشه اشغال می‌کند. ساخت تست با
        // ردیابیِ خاموش صریحاً رد می‌شود تا مدیر اول ردیابی را روشن کند.
        if (!class_exists('FWS_Tracker') || !FWS_Tracker::tracking_enabled()) {
            return array(false, 'ردیابی قیف تبدیل در تنظیمات خاموش است؛ تست A/B بدون ثبت رخدادها هرگز به نتیجه نمی‌رسد. ابتدا ردیابی را روشن کنید، سپس تست بسازید.');
        }
        // نسخهٔ ۲.۱۳ (I-49): تست «فقط-رنگی» وقتی استایل‌های افزونه خاموش است، نسخهٔ B هیچ
        // تفاوت دیداری ندارد (رنگ اصلاً منتشر نمی‌شود) و تست بی‌معنا اجرا می‌شود. تست
        // عنوان‌محور همچنان مجاز است (مستقل از استایل کار می‌کند).
        if ('' === $title_b && !FWS_Style_Manager::styles_enabled()) {
            return array(false, 'استایل‌های افزونه در تنظیمات خاموش‌اند؛ رنگ نسخهٔ B منتشر نمی‌شود و تستِ فقط-رنگی هیچ تفاوت دیداری‌ای ندارد. ابتدا استایل را روشن کنید یا تستِ تغییرِ عنوان بسازید.');
        }
     ...
     * نسخهٔ ۲.۱۰.۴ (B-32): ردیف‌های رخدادِ همین تست نیز پاک می‌شوند؛ قبلاً فقط ردیف گزینه
     * حذف می‌شد و ردیف‌های جدول رخدادها «یتیم» می‌ماندند و جدول را بی‌دلیل سنگین می‌کرد.
     * نسخهٔ ۲.۱۲.۶ (G-07): رخدادهای مرور از این نسخه شناسهٔ تستِ خود را حمل می‌کنند
     * (ستون test_id)؛ حذف اول بر اساس test_id است — امن حتی وقتی دو تستِ یک ویجت از نظر
     * زمانی هم‌پوشان‌اند و قبلاً حذفِ بازه‌ای، دادهٔ تستِ دیگر را هم می‌زد. برای ردیف‌های
     * قدیمیِ بدون مهر، فقط پنجرهٔ زمانیِ همین تست با شرط test_id = '' اجرا می‌شود.
     * ردیف‌های «خرید» (درآمد واقعی سفارش‌ها) دیگر حذف نمی‌شوند تا گزارش درآمد دروغ نگوید.
     * اگر بودجهٔ زمانی ۱۵ ثانیه‌ای تمام شود، ادامهٔ حذف به کرونِ تک‌موردی سپرده می‌شود
     * (G-19) تا ردیف‌ها دیگر هرگز یتیم نمانند.
    ...
    /**
     * پاکسازی دسته‌ای ردیف‌های رخداد یک تستِ حذف‌شده (B-32؛ بازنویسی G-07/G-19)
    ...
    private static function delete_test_events($test) {
        ...
        $over_budget = false;
        do {
            // ۱) ردیف‌های مهرخورده با شناسهٔ همین تست (از ۲.۱۲.۶) — مستقل از پنجرهٔ زمانی
            $affected = (int) $wpdb->query($wpdb->prepare(
                "DELETE FROM {$table}
                 WHERE test_id = %s AND event_type IN ('impression','add_to_cart','coupon')
                 LIMIT 5000",
                sanitize_text_field((string) $test['id'])
            ));
            // ۲) ردیف‌های قدیمیِ بدون مهر (پیش از ۲.۱۲.۶) — فقط پنجرهٔ همین تست و فقط test_id خالی؛
            // ردیف‌های مهرخوردهٔ تست‌های دیگر هرگز قربانی نمی‌شوند.
            if ($affected < 5000) {
                $affected += (int) $wpdb->query($wpdb->prepare(
                    "DELETE FROM {$table}
                     WHERE widget = %s AND variant IN ('A','B') AND test_id = ''
                       AND event_type IN ('impression','add_to_cart','coupon')
        ...
        // نسخهٔ ۲.۱۲.۶ (G-19): بودجهٔ زمانی تمام شد → ادامه با رویداد کرون تک‌موردی؛
        // قبلاً باقی‌مانده «هرگز» پاک نمی‌شد و در جدول رها می‌شد.
        if ($over_budget) {
            wp_schedule_single_event(time() + 120, 'fws_ab_delete_test_events_resume', array($test));
        }
    }

    /**
     * نسخهٔ ۲.۱۲.۶ (G-19): ادامهٔ حذف ردیف‌های تستِ حذف‌شده از کرون؛ تا وقتی ردیفی
     * مانده، خودش را دوباره زمان‌بندی می‌کند (بچ ۵۰۰۰تایی، بودجهٔ ۱۵ ثانیه در هر پاس).
     * @param array $test
     */
    public static function resume_test_events_deletion($test) {
        if (!is_array($test) || empty($test['id'])) {
            return;
        }
        self::delete_test_events($test);
    }
    ...
    public static function init() {
        ...
        // نسخهٔ ۲.۱۲.۶ (G-19): ادامهٔ حذف ردیف‌های تست‌هایی که حذفشان با سقف زمانی نیمه‌کاره ماند
        add_action('fws_ab_delete_test_events_resume', array(__CLASS__, 'resume_test_events_deletion'));
     ...
     * نسخهٔ ۲.۱۲.۶ (G-04): no-cache دیگر «سراسری» نیست — فقط صفحه‌هایی که ویجتِ تحت
     * تست واقعاً می‌توانند رندر کنند کش‌ناپذیر می‌شوند (نگاشت ویجت → تگ‌های شرطی ووکامرس؛
     * با فیلتر fws_ab_test_affects_request توسعه‌پذیر). یک تست روی ویجتِ سبد، دیگر کشِ
     * صفحهٔ اول/وبلاگ/محصولات سایت را هم نمی‌کُشد. اسلاگ ناشناخته = رفتار محافظه‌کارانهٔ
     * سراسری (ایمنیِ قبل حفظ می‌شود).
     * @return void
     */
    public static function maybe_send_nocache_headers() {
        ...
        foreach (self::get_tests() as $test) {
            if ('running' === $test['status'] && self::test_affects_current_request($test)) {
    ...
    /**
     * نسخهٔ ۲.۱۲.۶ (G-04): آیا این درخواست صفحه‌ای است که ویجتِ تحت تست می‌تواند رندر کند؟
     * نگاشت اسلاگ‌های قابل‌تست به تگ‌های شرطی رسمی ووکامرس/وردپرس — هم‌راستا با جاهایی
     * که واقعاً رندر می‌شوند (هوک‌های کلاسیک، بلوک سبد F-11، بنر جستجو F-32، شورت‌کدها).
     * شورت‌کد در محتوای دلخواه = نقطهٔ کورِ شرطی‌ها؛ توسعه‌دهنده می‌تواند با فیلتر
     * fws_ab_test_affects_request آن صفحه‌ها را هم کش‌ناپذیر کند.
     * @param array $test
     * @return bool
     */
    private static function test_affects_current_request($test) {
        $widget = isset($test['widget']) ? sanitize_key((string) $test['widget']) : '';
        switch ($widget) {
            case 'product':
                $affects = function_exists('is_product') && is_product();
                break;
            case 'cart':
                $affects = function_exists('is_cart') && is_cart();
                break;
            case 'thankyou':
                $affects = function_exists('is_order_received_page') && is_order_received_page();
                break;
            case 'shipping':
                // نوار ارسال در سبد و تسویه هر دو رندر می‌شود (سد اختلاف سبد → فیلرها)
                $affects = (function_exists('is_cart') && is_cart()) || (function_exists('is_checkout') && is_checkout());
                break;
            case 'account':
                $affects = function_exists('is_account_page') && is_account_page();
                break;
            case 'search':
                $affects = function_exists('is_search') && is_search();
                break;
            case 'exit_modal':
                // مودال خروج فقط در سبد/تسویه رندر می‌شود (گارد render_exit_intent_rescue_modal)
                $affects = (function_exists('is_cart') && is_cart()) || (function_exists('is_checkout') && is_checkout());
                break;
            default:
                $affects = true; // اسلاگ ناشناخته — محافظه‌کارانه مثل قبل
                break;
        }
        return (bool) apply_filters('fws_ab_test_affects_request', $affects, $test);
    }
}
