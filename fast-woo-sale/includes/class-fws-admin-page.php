...
class FWS_Admin_Page {
        ...
        ",
                ...
                foreach ( (array) $rows as $row ) {
                        // نسخهٔ ۲.۱۲.۶ (G-24): عنوان خامِ دیتابیس (با تگ/آنتینیِ احتمالی) نه
                        $results[] = array(
                                'id'   => (int) $row->ID,
                                'text' => html_entity_decode( wp_strip_all_tags( (string) $row->post_title ), ENT_QUOTES, 'UTF-8' ),
                        );
                }
        ...
        public function ajax_add_manual_rule() {
                ...
                // نسخهٔ ۲.۱۲.۵ (F-42): خواندن→بررسی تکراری/سقف→افزودن→نوشتن، همگی داخل قفل
                // اتمی با خوانش تازه — دو ادمین همزمان دیگر نوشتن یکدیگر را پس نمی‌گیرند.
                $fws_dup = false;
                $fws_cap = false;
                $fws_count = 0;
                FWS_Settings::update_list(
                        'manual_rules',
                        function ( $rules ) use ( $source, $target, $confidence, &$fws_dup, &$fws_cap, &$fws_count ) {
        ...
        public function ajax_delete_manual_rule() {
                ...
                // نسخهٔ ۲.۱۲.۵ (F-42): حذف اتمیک داخل قفل با خوانش تازه (توضیح در ajax_add_manual_rule).
                $fws_removed = false;
                $fws_kept_count = 0;
                FWS_Settings::update_list(
                        'manual_rules',
                        function ( $rules ) use ( $source, $target, &$fws_removed, &$fws_kept_count ) {
                                $kept    = array();
                                $removed = 0;
                                foreach ( $rules as $rule ) {
        ...
        public function ajax_add_blacklist_product() {
                ...
                // نسخهٔ ۲.۱۲.۵ (F-36): سقف ۲۰۰تاییِ sanitize_blacklist_ids «اولین» ۲۰۰ را نگه
                // می‌دارد؛ آیتمِ تازه‌افزوده در جایگاه ۲۰۱ بی‌صدا حذف می‌شد — پیام موفقیت
                // چشمک می‌زد ولی پس از reload چیپ نبود و محصول همچنان پیشنهاد می‌شد.
                // نسخهٔ ۲.۱۲.۵ (F-42): افزودن اتمیک داخل قفل با خوانش تازه.
                $fws_dup = false;
                $fws_cap = false;
                $fws_count = 0;
                FWS_Settings::update_list(
                        'product_blacklist',
                        function ( $list ) use ( $pid, &$fws_dup, &$fws_cap, &$fws_count ) {
                ...
                if ( $fws_cap ) {
                        wp_send_json_error( array( 'message' => 'سقف ۲۰۰ محصولِ لیست سیاه پر شده است؛ ابتدا چند مورد را حذف کنید.' ) );
                }
                FWS_Database_Miner::purge_cache();
                wp_send_json_success(
                        array(
        ...
        public function ajax_remove_blacklist_product() {
                ...
                FWS_Settings::update_list(
                        'product_blacklist',
                        function ( $list ) use ( $pid, &$fws_count ) {
        ...
        private function render_revenue_report_card() {
                ...
                <div class="card fws-admin-card" id="fws-revenue-card">
                        ...
                        <?php if ( ! empty( $report['mixed_currency'] ) && ! empty( $report['revenue_by_currency'] ) && count( $report['revenue_by_currency'] ) > 1 ) : ?>
                                <p style="padding:12px 16px; background:#eff6ff; border:1px solid #93c5fd; border-radius:8px;">در این بازه، ردیف‌های درآمد با ارزهای مختلف ثبت شده‌اند (<?php echo esc_html( implode( '، ', array_map( function ( $fws_cur ) { return ( '' === $fws_cur ) ? 'پیش از ارتقای شِما (ارز نامشخص)' : $fws_cur; }, array_keys( $report['revenue_by_currency'] ) ) ) ); ?>). جمع‌های این جدول فقط مبلغ‌های هم‌ارزِ ارزِ فعلی فروشگاه را می‌شمارند تا مبلغ‌های ناهم‌ارز (مثلاً پیش و پس از تغییر ریال↔تومان) بی‌صدا با هم جمع نشوند. جمعِ خامِ هر ارز در آرایهٔ گزارش (revenue_by_currency) موجود است.</p>
                        <?php endif; ?>
        ...
        private function render_settings_card( $settings ) {
                ...
                <div class="card fws-admin-card">
                        ...
                        <table class="form-table" role="presentation">
                                ...
                                <tr>
                                        <th scope="row"><label for="fws-upsell-shipping">سیاست ارسال سفارش فرعی آپسل</label></th>
                                        <td>
                                                <select id="fws-upsell-shipping" name="fws_prediction_settings[upsell_shipping]" class="regular-text">
                                                        <option value="none" <?php selected( $settings['upsell_shipping'], 'none' ); ?>>بدون هزینهٔ ارسال (ارسال همراه سفارش اصلی — پیش‌فرض)</option>
                                                        <option value="parent" <?php selected( $settings['upsell_shipping'], 'parent' ); ?>>کپی خط ارسال سفارش اصلی (همان روش، همان مبلغ)</option>
                                                        <option value="recalculate" <?php selected( $settings['upsell_shipping'], 'recalculate' ); ?>>محاسبهٔ واقعی هزینهٔ ارسال تک‌کالا با روش‌های ووکامرس</option>
                                                        <option value="flat" <?php selected( $settings['upsell_shipping'], 'flat' ); ?>>هزینهٔ ثابت (فیلد بعدی)</option>
                                                </select>
                                                <p class="description">نسخهٔ ۲.۱۳ — کالای فیزیکیِ آپسلِ سفارشِ وابسته قبلاً همیشه بدون خطِ ارسال ثبت می‌شد. سیاست را شفاف انتخاب کنید؛ در هر حالت، سیاست اعمال‌شده صریحاً در یادداشت سفارشِ فرعی ثبت می‌شود. کالای مجازی/دانلودی همیشه بدون ارسال است.</p>
                                                <div style="margin-top:8px;">
                                                        <label for="fws-upsell-shipping-cost">هزینهٔ ثابت ارسال (فقط برای حالت «هزینهٔ ثابت»):</label>
                                                        <input type="number" id="fws-upsell-shipping-cost" name="fws_prediction_settings[upsell_shipping_cost]" value="<?php echo esc_attr( $settings['upsell_shipping_cost'] ); ?>" min="0" step="any" class="small-text" style="margin-inline-start:8px;">
                                                </div>
                                        </td>
                                </tr>
                                <tr>
